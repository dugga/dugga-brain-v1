...
public class GA4BSendJ {
	 ...
	 * Constructor.
	 *
	 * @param DocsService {@link DocsService} for interface to DocsService API service.
	...
	// Prepare file for upload.  Our example only uploads 1 file, but could easily be changed to upload multiple files.
	// Here, we add just the one file to a List, then we process that List.
	private static void executeUpload(String fromFIle)
	...
	/**
	 * Uploads given collection of files.  The call blocks until all uploads are done.
	 *
	 * @param url create-session url for initiating resumable uploads for documents API.
	 * @param files list of absolute filepaths to files to upload.
	 * @param chunkSize max size of each upload chunk.
	 */
	public static Collection<DocumentListEntry> uploadFiles(String url,List<String> files, int chunkSize) throws IOException, ServiceException, InterruptedException {
		GA4BSendJ.FileUploadProgressListener listener = new GA4BSendJ(docsService, output).new FileUploadProgressListener(); 
		...
		for (String fileName : files) {
			...
			ResumableGDataFileUploader uploader = new ResumableGDataFileUploader.Builder(docsService, new URL(url), mediaFile, null)
		...
		public synchronized void progressChanged(ResumableHttpFileUploader uploader)
		{
			...
			switch(uploader.getUploadState()) {
				...
				case CLIENT_ERROR:
					...
					break;
				case IN_PROGRESS:
					output.println(fileId + ":" + String.format("%3.0f", uploader.getProgress() * 100) + "%");
					break;
				case NOT_STARTED:
		...
		public synchronized boolean isDone() {
			...
			for (ResumableGDataFileUploader uploader : trackedUploaders) {
				...
				switch(uploader.getUploadState()) {
					case COMPLETE:
						try {
							DocumentListEntry entry = uploader.getResponse(DocumentListEntry.class);
							uploaded.put(fileId, entry);
						} catch (IOException e) {
							failed.put(fileId, "Upload completed, but unexpected error reading server response");
						} catch (ServiceException e) {
							failed.put(fileId, "Upload completed, but failed to parse server response");
						}
						break;
					case CLIENT_ERROR:
